...
% under "03", with \ttfamily scoped to the number so it cannot leak.
%
% The title's weight and size go through the node's font= key, not inline
% switches in the text: TikZ re-establishes the node font at each \\, so an
% inline \bfseries applies to the first line only and "decomposition" comes out
% regular.  font= must name the family as well -- it replaces the picture's
% font=\sffamily rather than adding to it, so \sffamily\bfseries\small, or the
% head silently reverts to roman.
%
...
\node[anchor=north west, xshift=\stagenumwd, align=left,
      font=\sffamily\bfseries\small] at (129, 86.5)
  {\color{ink}Differential Thomas\\decomposition of Ansatz};
...
% ---------- 06 CONSLEVELS ----------
\node[anchor=west] at (101, 46) {\stagehead{6}{Combine and Simplify}};
...
% cards x 182..231
% ==================================================================
% Through \stagehead, so the number's font and its gap match 01--06 by
% construction; the node still starts at the cards' left edge, which is where
% the other stages put their numbers.
\node[anchor=north west] at (182, 86.5)
  {\stagehead{7}{Solutions}%
...
\node[anchor=west] at (184, 28.5)
  {\fontsize{5.2pt}{6.4pt}\selectfont$v = r$};
\node[anchor=south west] at (184, 24)
  {\note{all s shells \textperiodcentered\ second-order}};
...
\node[anchor=south west] at (184, 5)
  {\note{separable in parabolic coordinates}};
